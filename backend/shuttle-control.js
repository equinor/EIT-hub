    ...
    start(){
        ...
        self.browserWs.onTopic('input', function(message)  {
            ...
            if (browserId >= self.currentBrowser) {
                ...
                // Check that body contains only x, y, z, r
                const neededKeys = ['x', 'y', 'z', 'r'];
                if (Object.keys(input).every(key => neededKeys.includes(key))) {
                    self.deviceWs.sendMessage('shuttle', input);
                } else {
